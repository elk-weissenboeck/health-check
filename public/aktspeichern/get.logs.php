...
// NEU: liefert zusätzlich fileExists pro Eintrag.
...
$sql = "
  SELECT id, machine, user, sessionId, logType, logDate, fileName, filePath,
         sizeBytes, checksum, uploadedUtc
  FROM logs
";
...
try {
    ...
    // NEU: fileExists ergänzen
    foreach ($rows as &$r) {
        $rel = $r['filePath'] ?? '';
        $abs = __DIR__ . '/' . ltrim($rel, '/');
        $r['fileExists'] = ($rel !== '' && is_file($abs));
    }
    unset($r);
